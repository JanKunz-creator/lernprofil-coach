(() => {
  ...
  function renderPretest(unitState, topic) {
    ...
    nextButton.addEventListener("click", () => {
      if (!record.answers[question.id]) return;
  ...
  function renderAuditoryLearning(unitState, topic) {
    ...
    app.innerHTML = `
      ...
      <section class="audio-stage">
        ...
        <audio id="learningAudio" preload="metadata">
          <source src="${esc(learning.audioSrc)}" type="audio/mpeg">
        </audio>
    ...
    audio.addEventListener("error", () => {
      const expectedFile = String(learning.audioSrc || "")
        .split("/")
        .pop()
        .split("?")[0];

      error.textContent =
        `Die Audioaufnahme konnte nicht geladen werden. Erwartete Datei: ${expectedFile}. ` +
        "Bitte Seite neu laden oder auf „Erneut laden“ tippen.";
      error.hidden = false;
      control.disabled = false;
      control.textContent = "Erneut laden";
      control.dataset.reloadAudio = "true";
    });

    audio.addEventListener("canplay", () => {
      error.hidden = true;
      delete control.dataset.reloadAudio;
      control.disabled = false;
      if (audio.paused) {
        control.textContent = savedPosition > 0 ? "Aufnahme fortsetzen" : "Aufnahme starten";
      }
    });
    ...
    control.addEventListener("click", async () => {
      if (control.dataset.reloadAudio === "true") {
        error.hidden = true;
        control.textContent = "Wird geladen …";
      ...
      if (audio.paused) {
        try {
          ...
          error.textContent =
            "Die Aufnahme konnte nicht gestartet werden. Bitte „Erneut laden“ wählen oder die Seite neu öffnen.";
          error.hidden = false;
          control.disabled = false;
          control.textContent = "Erneut laden";
          control.dataset.reloadAudio = "true";
  ...
  function renderTransfer(topic) {
    ...
    const nextTransferButton = document.querySelector("#nextTransferButton");

    bindChoices(value => {
      testState.transferAnswers[item.id] = value;
      saveState();
      if (nextTransferButton) nextTransferButton.disabled = false;
    });

    nextTransferButton.addEventListener("click", () => {
      if (!testState.transferAnswers[item.id]) return;
  ...
  function choiceMarkup(option, selected) {
    ...
    return `
      ...
      >${esc(option.text)}</button>
    `;
  }

  function bindChoices(onSelect) {
    const buttons = Array.from(document.querySelectorAll("[data-choice-value]"));
    ...
    buttons.forEach(button => {
      button.addEventListener("click", event => {
        ...
        buttons.forEach(item => {
          const selected = item.dataset.choiceValue === value;
          item.classList.toggle("selected", selected);
          item.setAttribute("aria-pressed", selected ? "true" : "false");
        });
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      navigator.serviceWorker
        .register("./service-worker-034.js?v=034", { updateViaCache: "none" })
